...
import com.github.chenlijia1111.utils.http.netty.webSocket.pojo.VideoMessage;
 ...
 * @since 2020/4/5 8:40
 */
public class HandleVideoMessage implements IHandleMessage {
    ...
    public void handleMessage(Channel channel, TextWebSocketFrame webSocketFrame) {
        if (null != channel && null != webSocketFrame) {
            ...
            if (StringUtils.isNotEmpty(text) &&
                    Objects.equals(MessageTypeEnum.VIDEO.getType(),
                            JSONUtil.strToJsonNode(text).get("type").asText())) {
                //是视频操作，开始处理
                VideoMessage videoMessage = JSONUtil.strToObj(text, VideoMessage.class);
                if (StringUtils.isNotEmpty(videoMessage.getToUser())) {
                    //这里只处理点对点的消息，如果要处理群组消息，需要自己实现，因为在内存里存太多数据不好。
                    //查找channel发送消息
                    ChannelId toUserChannelId = MessageDealService.USER_TO_CHANNEL_LONG_KEY.get(videoMessage.getToUser());
